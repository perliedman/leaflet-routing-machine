describe('L.Routing.Itinerary', function() {
	describe('._round', function() {
		it('rounds 0 < x < 5 to multiple of 0.5', function() {
			var p = new L.Routing.Itinerary(undefined);
		...
		it('considers rounding sensitivity', function() {
			var p = new L.Routing.Itinerary({roundingSensitivity: 5});
			expect(p._round(24)).toBe(24);
			expect(p._round(52)).toBe(50);
		});
	});

	describe('._formatDistance', function() {
		it('rounds long distances reasonably', function() {
			var p = new L.Routing.Itinerary({
				distanceTemplate: '{value}'
			});
			expect(parseInt(p._formatDistance(22000), 10)).toBe(20);
			expect(parseInt(p._formatDistance(24000), 10)).toBe(25);
			expect(parseInt(p._formatDistance(86000), 10)).toBe(90);
		})
	});
});
